...
with col2:
    total_arrests=data[data['is_arrested']==True]
    st.metric("🔗Total Arrests",len(total_arrests))
with col3:
...
query_map={
     ...
     "⚖️violation that rarely results in search or arrest":"select violation, count(*) as total_stops, sum(case when search_conducted = true then 1 else 0 end) as total_searches, sum(case when is_arrested = true then 1 else 0 end) as total_arrests, round(100.0 * sum(case when search_conducted = true then 1 else 0 end) / count(*), 2) as search_rate_percentage, round(100.0 * sum(case when is_arrested = true then 1 else 0 end) / count(*), 2) as arrest_rate_percentage from traffic_logs group by violation order by arrest_rate_percentage, search_rate_percentage limit 3",
